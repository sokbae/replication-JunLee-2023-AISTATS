########################################################################## 
####  Plug-in retrospective logit estimation of                       ####
...
Logit_retro = function(outcome=outcome,treat=treat,X=X){
  ...
  ### Estimation of Pr(treat=1|X=x,outcome=1) using logit ###   
  fit_case = glm(formula = treat[outcome==1] ~ X[outcome==1,], 
                 family = "binomial")
  ...
  ### Estimation of Pr(treat=1|X=x,outcome=0) using logit ###     
  fit_control = glm(formula = treat[outcome==0] ~ X[outcome==0,], 
                    family = "binomial")
